...
void Initialize(void)
{
    ...
    display_string[0] = '>';
    display_string[1] = '>';
    display_string[dispPos] = '<';
    display_string[dispPos+1] = '<';
    for(k=4; k < 21; k++){
        display_string[k] = ' '; 
    }
    direction = true; 
...
void RunLogic(void)
{
    ...
    if (dispPos <= 17){  
        if(cmd >= 33 && cmd <= 122 )
        {
            ...
            display_string[dispPos] = cmd; 
            display_string[(dispPos+1)] = '<'; 
            display_string[(dispPos+2)] = '<';
            dispPos++; 
        }
    }    
    cmd = 0;
    if( direction == false) {
        index_count -- ;
        if (index_count < 0){
            index_count = display_size - 1 ; 
        }
    }    
    else {
        index_count ++;
        if (index_count > display_size - 1)
            index_count = 0; 
    }
...
void DrawScreen(void)
{
    ...
    {   
        MacUILib_printf("%c", display_string[(index_count + j) % display_size]);
