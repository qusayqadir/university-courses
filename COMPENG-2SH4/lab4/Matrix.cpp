...
Matrix::Matrix()
{
	...
	{
		matrixData[i] = new int[colsNum]; //allocates memory for an array of columns to create memory for a 2D matrix
	}
...
Matrix Matrix::subMatrix(int i, int j){
	...
	Matrix subMat(i+1,j+1); 
	for(int rows = 0; rows <=i; rows++){
		for (int cols = 0; cols <=j; cols++)
		{
			subMat.setElement(getElement(rows,cols),rows,cols); 
		}
	}
	return subMat; 
...
string Matrix::toString(){
	...
	// then use the to_string built in funciton to add the \n after that
	//must be two nested for loops
