...
char **read_words(const char *input_filename, int *nPtr){
    ...
    //          This is the number of valid lines in the text file containing words to be sorte


    fscanf(file, "%d", nPtr);   // stores number of words in nPtr 
    // Step 3:  Allocate an array of nPtr number of char pointer under word_list


    word_list = malloc(sizeof(char *) * (*nPtr)); 
    int i, j; 
    ...
    for(i = 0; i < *nPtr; i++){
        ...
        while(buffer[string_length] != '\0')
        {
            string_length ++; 
        }
        word_list[i] = malloc((string_length) * sizeof(char));
