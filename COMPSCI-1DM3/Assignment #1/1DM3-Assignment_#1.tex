...
Use Theorem 2 on (n! + $5^n$ ), for n! big-O is $O(n!)$ and  big-O for $5^n$  is $O(5^n)$. For (n! + $5^n$ ) big-O estimate is $O(5^n)$ as $O(5^n) > O(n!) $ \\\\
Use Theorem 3 on ( $n^n + n2^n + 5^n$) (n! + $5^n$ ) the final big-O estimate for the entire function is $O(5n^n)$
\newpage
