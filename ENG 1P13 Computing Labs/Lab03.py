...
def extract_grade(x):
    k = x.split("-") #splits the list into two where it find "-"
    #k = float(k[1].strip("%"))
    return (float(k[1].strip("%"))) # goes to the first index, and removes where it finds "%"
...
def class_average(final_marks): 
    z = 0
    for i in final_marks: # a for loop to determine average of the list passed through when calling the function, and summing the numbers dependent on the length of the list
        l = (extract_grade(i))
        z += l
    return (z/len(final_marks)) 
...
def class_std_dev(final_marks):
    average = class_average(final_marks) #finding the average of the class
    l = []
    k = 0 
    for i in final_marks: ##creating a list of just the class grades
        l.append(extract_grade(i))
    for i in l: ## a for loop, for sumamtion as long as the length of the list to determine its value
        k += (i-average)**2
    return (k/len(l))**(1/2) ## second half of the summation equation. 
...
def main():
    final_marks = ["400123848-71%","1001425-99.5%","1009980-8.2%","1001480-84.0%","400199444-87.9%"]
    avg = class_average(final_marks)
    std_dev = class_std_dev(final_marks)
...
#  ```
# DOUBLE CLICK TO EDIT THIS CELL. DO NOT ERASE THE TRIPLE QUOTATION MARK
# 
# 1. If the input does not follow the format of strings for the entires the code will not run when dash is omittedd. This is because this wil cause the function to not be able to seperate the two values, student number, and grade, through its string manipulation code. Furthermore, the next line would not be able to find index 1, since the string did not split, the index of the string is zero. Hence causing the program to fail. 
# 
# 2. The only thing that would be changed would be the string_name.split(":"), the value to passed through the string split function would change, and the float of the index 1 would need to be returned without using the string_name.strip() function. Previously, this would remove the string component from index1 so its value can be converted to a float.  
# 
# 3. For average it is possible to use a single loop only if there was no extract_grade() function. Other wise, 
# computing the average and standard deviation grade using a single loop is not possible, this is becasuse not only for the average but the standard deviation to be calculated it requires a value, x, to go through summation. For this to happen, in this case, x is added to other grades one at a time, and thus requires the for loop. 
# ```
